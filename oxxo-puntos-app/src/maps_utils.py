...
La aplicación recibe enlaces que pueden ser URL completas de Google Maps, enlaces cortos maps.app.goo.gl, URLs de Bing/Waze o hipervínculos de Excel cuyo texto visible no es la URL real. Este módulo extrae pares latitud/longitud sin invertirlos, resuelve redirecciones cuando es necesario y deja una fuente auditable para cada coordenada. """ from future import annotations 

import html as html_lib import json import re from concurrent.futures import ThreadPoolExecutor, as_completed from functools import lru_cache from pathlib import Path from typing import Iterable from urllib.parse import parse_qs, unquote, unquote_plus, urlparse 

import requests import streamlit as st 

USER_AGENT = "oxxo-puntos-app/2.0 (contacto: equipo-expansion@oxxo.com)" REQUEST_TIMEOUT = (4, 10) 
...
GOOGLE_PREVIEW_TIMEOUT = (3, 6) MAX_LINK_WORKERS = 6 APP_ROOT = Path(file).resolve().parents[1] COORDINATE_CACHE_PATH = APP_ROOT / "data" / "maps_coordinate_cache.json" 
...
SHORT_LINK_DOMAINS = {"maps.app.goo.gl", "goo.gl"} MAP_HOST_HINTS = ("google.", "goo.gl", "share.google", "bing.com", "waze.com") 

Llaves de query frecuentes para pares latitud,longitud. Incluye Google, 

Bing y enlaces compartidos de aplicaciones móviles. 

COORD_QUERY_KEYS = ( "q", "query", "ll", "destination", "origin", "center", "location", "coords", "coordinates", "latlng", "latlon", ) 

def _clean_url(value: object) -> str: """Normaliza una URL preservando su contenido semántico. 
...
def _valid_coordinates(lat: object, lon: object) -> tuple[float, float] | None: """Convierte y valida una pareja geográfica en el orden latitud,longitud.""" try: latitude, longitude = float(lat), float(lon) except (TypeError, ValueError): return None if -90 <= latitude <= 90 and -180 <= longitude <= 180: return latitude, longitude return None 

@lru_cache(maxsize=1) def _verified_coordinate_cache() -> dict[str, dict[str, object]]: """Carga coordenadas auditadas del libro actual, si el archivo existe.""" try: payload = json.loads(COORDINATE_CACHE_PATH.read_text(encoding="utf-8")) entries = payload.get("entries", {}) return entries if isinstance(entries, dict) else {} except (OSError, ValueError, TypeError): return {} 

def _cached_coordinates(url: str) -> tuple[float, float, str] | None: """Devuelve una coordenada auditada para el mismo enlace normalizado.""" entry = _verified_coordinate_cache().get(_clean_url(url)) if not isinstance(entry, dict): return None coordinates = _valid_coordinates(entry.get("lat"), entry.get("lon")) if not coordinates: return None source = str(entry.get("source") or "Coordenada auditada del enlace") return coordinates[0], coordinates[1], source 

def _parse_coordinate_pair(value: object) -> tuple[float, float] | None: """Extrae un par latitud,longitud de una cadena de consulta.""" if not isinstance(value, str): return None text = unquote(value).strip() # Soporta valores como geo:4.71,-74.07 o loc:4.71,-74.07. match = re.search( r"(?:geo:|loc:)?\s*(-?\d{1,2}(?:.\d+)?)\s*[,~;]\s*" r"(-?\d{1,3}(?:.\d+)?)", text, ) if not match: return None return _valid_coordinates(match.group(1), match.group(2)) 

def _parse_coords_from_text(value: object) -> tuple[float, float] | None: """Obtiene coordenadas explícitas de formatos habituales de URL de mapas.""" link = _clean_url(value) if not link: return None 
...
def _hostname(url: str) -> str: try: return urlparse(url).netloc.casefold().split(":", 1)[0] except ValueError: return "" 

def _is_short_link(url: str) -> bool: host = _hostname(url) return host in SHORT_LINK_DOMAINS or host.endswith(".goo.gl") 

def _should_follow_redirects(url: str) -> bool: host = _hostname(url) return bool(host) and any(hint in host for hint in MAP_HOST_HINTS) 

def _map_query_as_address(url: str) -> str: """Extrae una búsqueda o dirección legible de una URL final de mapas. 
...
def _google_place_id(url: str) -> str: """Extrae el identificador 0x...:0x... de una ficha de Google Maps.""" link = _clean_url(url) if not link: return "" decoded = unquote(link) parsed = urlparse(decoded) for candidate in parse_qs(parsed.query).get("ftid", []): if re.fullmatch(r"0x[0-9a-f]+:0x[0-9a-f]+", candidate, flags=re.IGNORECASE): return candidate match = re.search(r"(?:!1s|ftid=)(0x[0-9a-f]+:0x[0-9a-f]+)", decoded, flags=re.IGNORECASE) return match.group(1) if match else "" 

def _google_preview_payload(place_id: str, label: str) -> str: """Construye el parámetro de vista previa usado por una ficha pública. 
...
@lru_cache(maxsize=2_000) def google_place_coordinates(url: str) -> tuple[float, float] | None: """Obtiene el pin principal de una ficha pública de Google Maps. 
...
@lru_cache(maxsize=2_000) def resolve_map_link(url: str) -> str: """Sigue redirecciones sin descargar el cuerpo de la página de mapas. 
...
@lru_cache(maxsize=4_000) def geocode_address(address: str, region_hint: str = "Colombia") -> tuple[float, float] | None: """Geocodifica una dirección sólo si el enlace no expone coordenadas.""" normalized = (address or "").strip() if not normalized: return None try: response = requests.get( "https://nominatim.openstreetmap.org/search", params={"q": f"{normalized}, {region_hint}", "format": "json", "limit": 1}, headers={"User-Agent": USER_AGENT}, timeout=REQUEST_TIMEOUT, ) response.raise_for_status() data = response.json() if data: return _valid_coordinates(data[0].get("lat"), data[0].get("lon")) except (requests.RequestException, ValueError, KeyError, IndexError, TypeError): return None return None 

def get_coordinates(maps_link: str, address: str = "") -> tuple[float | None, float | None, str]: """Devuelve (latitud, longitud, fuente) de un enlace o una dirección. 
...
def get_coordinates_batch( records: Iterable[tuple[object, str, str]], max_workers: int = MAX_LINK_WORKERS ) -> dict[object, tuple[float | None, float | None, str]]: """Resuelve varios registros de forma acotada sin bloquear la interfaz. 
