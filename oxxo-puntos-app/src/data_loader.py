...
def _add_visit_coordinates(df: pd.DataFrame) -> tuple[str | None, str | None]:
    ...
    Para los registros de **Operaciones**, la URL real de Maps es la fuente
    principal: se decodifica la coordenada incluida en el enlace y se resuelve
    la URL final si se trata de un enlace corto. Las columnas Y/X y la dirección
    sólo se usan como respaldo. Un registro sólo se considera mapeable si
    ...
    # Las columnas Y/X se calculan ahora, pero se aplican después de la URL
    # para que el enlace de Maps tenga prioridad en los puntos de Operaciones.
    if "Y" in df.columns and "X" in df.columns:
        ...
        latitude_xy = longitude_xy = None
        xy_valid = pd.Series(False, index=df.index)

    # Fuente principal: enlace real de Maps; la dirección del registro se deja
    # fuera de esta primera pasada para no adelantarla al respaldo Y/X.
    # Se atienden de forma acotada para que los enlaces cortos no bloqueen el
    # mapa durante una recarga.
    ...
    # Respaldo 1: columnas explícitas Y/X, sólo para las operaciones cuya URL
    # no produjo una pareja completa y válida.
    needs_xy_fallback = df["lat"].isna() | df["lon"].isna()
    xy_fallback = xy_valid & needs_xy_fallback
    if xy_fallback.any():
        df.loc[xy_fallback, "lat"] = latitude_xy.loc[xy_fallback]
        df.loc[xy_fallback, "lon"] = longitude_xy.loc[xy_fallback]
        df.loc[xy_fallback, "fuente_coordenadas"] = "Columnas Y/X de Operaciones (respaldo)"

    # Respaldo 2: la dirección sólo se geocodifica si la URL y las columnas
    # Y/X no produjeron una pareja válida.
    address_records = []
    if address_col:
        for index, row in df.iterrows():
            link = _cell_to_text(row.get("enlace_maps_leido", ""))
            address = _cell_to_text(row.get(address_col, ""))
            needs_address_fallback = pd.isna(df.at[index, "lat"]) or pd.isna(df.at[index, "lon"])
            if address and needs_address_fallback:
                address_records.append((index, "", address))

    for index, (latitude, longitude, source) in get_coordinates_batch(address_records).items():
        if latitude is not None and longitude is not None:
            df.at[index, "lat"] = latitude
            df.at[index, "lon"] = longitude
        df.at[index, "fuente_coordenadas"] = source
