...
MAP_HOST_HINTS = (
    "google.", "goo.gl", "share.google", "bing.com", "waze.com",
    "maps.apple", "apple.com",
)

# Llaves de query frecuentes para pares latitud,longitud. Incluye Google,
# Bing y enlaces compartidos de aplicaciones móviles.
# La operación está restringida a Colombia; estos límites se aplican sólo
# al respaldo por dirección, no a la lectura del enlace, que sigue siendo la
# fuente de verdad del pin compartido.
COLOMBIA_BOUNDS = (-5.5, 14.5, -82.5, -66.5)
...
def _valid_colombia_coordinates(lat: object, lon: object) -> tuple[float, float] | None:
    """Valida coordenadas del territorio operativo colombiano."""
    coordinates = _valid_coordinates(lat, lon)
    if not coordinates:
        return None
    latitude, longitude = coordinates
    south, north, west, east = COLOMBIA_BOUNDS
    return coordinates if south <= latitude <= north and west <= longitude <= east else None
...
def build_exact_maps_url(lat: object, lon: object) -> str:
    """Construye un enlace de Google Maps que abre exactamente el par recibido."""
    coordinates = _valid_coordinates(lat, lon)
    if not coordinates:
        return ""
    latitude, longitude = coordinates
    # Ocho decimales conservan precisión submétrica y evitan que Maps vuelva a
    # interpretar una dirección o el centro de la vista como el punto buscado.
    return (
        "https://www.google.com/maps/search/?api=1&query="
        f"{latitude:.8f}%2C{longitude:.8f}"
    )
...
def _parse_coords_from_text(value: object) -> tuple[float, float] | None:
    ...
    # El orden es deliberado: !3d/!4d y ppois/cp representan el pin,
    # mientras que @ suele ser sólo el centro de la ventana visible.
    patterns = (
        ...
        r"(?:[?&]|^)ppois=(-?\d{1,2}(?:\.\d+)?)(?:_|%5[fF]|~|%7[eE])(-?\d{1,3}(?:\.\d+)?)",
        r"(?:[?&]|^)cp=(-?\d{1,2}(?:\.\d+)?)(?:~|%7[eE]|_|%5[fF])(-?\d{1,3}(?:\.\d+)?)",
        # Google Maps puede abrir directamente /search/<lat>,<lon>.
        r"/(?:maps/)?(?:search|place)/\s*(-?\d{1,2}(?:\.\d+)?)\s*,\s*\+?\s*(-?\d{1,3}(?:\.\d+)?)",
        # Coordenada explícita incluida después de /place/.
        r"/place/\s*(-?\d{1,2}(?:\.\d+)?)\s*,\s*(-?\d{1,3}(?:\.\d+)?)",
        # Vista/resultado de Google Maps: .../@4.7101,-74.0721,15z.
        # Sólo se usa después de buscar formatos de pin inequívocos.
...
def _map_query_as_address(url: str) -> str:
    ...
    for _ in range(2):
        ...
        if next_value == decoded:
            break
        decoded = next_value
    # El path original conserva %23 como parte de la dirección; si se busca
    # después de decodificarlo, urlparse lo interpreta como fragmento y puede
    # truncar el nombre (por ejemplo, "#63d-36").
    ...
    match = re.search(
        r"/(?:maps/)?place/([^/]+)", parsed_original.path, flags=re.IGNORECASE
    )
    if match:
...
def google_place_coordinates(url: str) -> tuple[float, float] | None:
    ...
    # Google cambia con frecuencia la cantidad de campos que coloca entre el
    # pin y el ID. La coordenada sólo se acepta si aparece cerca del mismo
    # place_id, nunca por ser la primera pareja numérica de la respuesta.
    id_positions = [match.start() for match in re.finditer(re.escape(place_id), body, flags=re.IGNORECASE)]
    for coordinate in coordinate_pattern.finditer(body):
        nearby = body[max(0, coordinate.start() - 350): coordinate.end() + 350]
        same_place = place_id.casefold() in nearby.casefold()
        if same_place and any(abs(coordinate.start() - position) <= 350 for position in id_positions):
            valid = _valid_coordinates(coordinate.group(1), coordinate.group(2))
            if valid:
                return valid
    return None
    ...
    try:
        for attempt in range(2):
            try:
                ...
                resolved = response.url or link
                if resolved != link or attempt == 1:
                    return resolved
            except requests.RequestException:
                if attempt == 1:
                    return link
...
def geocode_address(address: str, region_hint: str = "Colombia") -> tuple[float, float] | None:
    ...
    try:
        ...
        data = response.json()
        if data:
            return _valid_colombia_coordinates(data[0].get("lat"), data[0].get("lon"))
