...
def _add_visit_coordinates(df: pd.DataFrame) -> tuple[str | None, str | None]:
    ...
    La prioridad es: enlace de mapa real (incluido el destino de
    hipervínculos de Excel), columnas explícitas Y/X válidas y, por último,
    geocodificación contextual de dirección. Así, cuando el enlace trae el pin,
    no se reemplaza por unas columnas antiguas o pertenecientes a otra captura.
    Un registro sólo se considera mapeable si dispone de **ambas** coordenadas
    dentro de rangos geográficos válidos.
    """
    ...
    # Se conserva el enlace recibido para auditoría; el enlace exacto se genera
    # después de validar lat/lon y nunca se sustituye silenciosamente por una
    # dirección ambigua.
    df["enlace_maps_leido"] = pd.Series("", index=df.index, dtype="object")
    df["enlace_maps_exacto"] = pd.Series("", index=df.index, dtype="object")
    ...
    from src.maps_utils import build_exact_maps_url, get_coordinates_batch

    # Fuente 1: el enlace real de la celda. Se resuelve antes que X/Y para que
    # el pin que el operador compartió sea la referencia canónica del registro.
    link_records = []
    for index, row in df.iterrows():
        link = _cell_to_text(row.get("enlace_maps_leido", ""))
        if link:
            link_records.append((index, link, ""))
    for index, (latitude, longitude, source) in get_coordinates_batch(link_records).items():
        if latitude is not None and longitude is not None:
            df.at[index, "lat"] = latitude
            df.at[index, "lon"] = longitude
        df.at[index, "fuente_coordenadas"] = source

    # Fuente 2: X es longitud y Y es latitud. Sólo se usa si el enlace no
    # resolvió el pin; nunca pisa coordenadas ya obtenidas del vínculo.
    if "Y" in df.columns and "X" in df.columns:
        ...
        needs_xy = (df["lat"].isna() | df["lon"].isna()) & xy_valid
        df.loc[needs_xy, "lat"] = latitude_xy.loc[needs_xy]
        df.loc[needs_xy, "lon"] = longitude_xy.loc[needs_xy]
        df.loc[needs_xy, "fuente_coordenadas"] = "Columnas Y/X de Operaciones (respaldo)"

    # Fuente 3: dirección contextual. Incluir nombre, plaza y región mejora la
    # precisión de direcciones incompletas como "Cra 13 a" sin inventar una
    # coordenada cuando el servicio no devuelve un resultado.
    needs_address = df["lat"].isna() | df["lon"].isna()
    if needs_address.any():
        records = []
        for index, row in df.loc[needs_address].iterrows():
            address_parts = []
            for column in (address_col, "Nombre del Punto", "Plaza", "Región", "Regional"):
                value = _cell_to_text(row.get(column, "")) if column else ""
                if value and value.casefold() not in {part.casefold() for part in address_parts}:
                    address_parts.append(value)
            contextual_address = ", ".join(address_parts)
            records.append((index, "", contextual_address))
    ...
    # Generar un vínculo canónico que abra exactamente la coordenada validada.
    # El enlace original queda disponible en `enlace_maps_leido` para auditarlo.
    for index, row in df.iterrows():
        exact_link = build_exact_maps_url(row.get("lat"), row.get("lon"))
        if exact_link:
            df.at[index, "enlace_maps_exacto"] = exact_link
    ...
    df.loc[~valid, "enlace_maps_exacto"] = ""
