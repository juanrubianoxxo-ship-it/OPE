...
from src.maps_utils import build_exact_maps_url
...
# ============================================================== PAGE 2 ====
    ...
    with col_info:
        ...
        maps_link_original = str(
            fila_visita.get("enlace_maps_leido")
            or fila_visita.get("Enlace de la ubicación en Google Maps", "")
            or ""
        ).strip()
        maps_link_exact = str(fila_visita.get("enlace_maps_exacto", "") or "").strip()
        if not maps_link_exact:
            maps_link_exact = build_exact_maps_url(
                fila_visita.get("lat"), fila_visita.get("lon")
            )
        if maps_link_exact:
            st.link_button("🗺️ Abrir coordenada exacta en Google Maps", maps_link_exact)
            st.caption(
                f"Enlace generado desde el pin validado · "
                f"{float(fila_visita['lat']):.8f}, {float(fila_visita['lon']):.8f}"
            )
            if maps_link_original and maps_link_original != maps_link_exact:
                with st.expander("Ver enlace original recibido desde OneDrive"):
                    st.link_button("Abrir enlace original", maps_link_original)
        elif maps_link_original:
            st.link_button("🗺️ Abrir enlace original de Maps", maps_link_original)
    ...
    if centro_mapa is not None:
        ...
        def enlace_popup_exacto(fila):
            """Devuelve un enlace HTML que abre la coordenada de ese marcador."""
            exact_link = str(fila.get("enlace_maps_exacto", "") or "").strip()
            if not exact_link:
                exact_link = build_exact_maps_url(fila.get("lat"), fila.get("lon"))
            if not exact_link:
                return ""
            return (
                f'<br><a href="{html.escape(exact_link, quote=True)}" '
                'target="_blank" rel="noopener noreferrer">Abrir coordenada exacta</a>'
            )
        ...
        if tiene_coordenadas_punto:
            ...
            popup_evaluado = folium.Popup(
                ...
                + enlace_popup_exacto(fila_visita),
                max_width=300,
            )
            if ICONO_PATH.exists():
        ...
        for _, punto_op in puntos_operacion_mapeables.iterrows():
            ...
            popup_op = folium.Popup(
                f"<b>⚫ {nombre_op}</b><br>Estado Growth: {estado_growth_val}<br>"
                f"Dirección: {direccion_op}{enlace_popup_exacto(punto_op)}",
                max_width=300,
            )
            folium.CircleMarker(
        ...
        for _, punto_subido in operaciones_growth_subidas.iterrows():
            ...
            popup_subido = folium.Popup(
                f"<b>🔴 {nombre_subido}</b><br>Estado Growth: Subido<br>"
                f"Dirección: {direccion_subido}{enlace_popup_exacto(punto_subido)}",
                max_width=300,
            )
            folium.CircleMarker(
